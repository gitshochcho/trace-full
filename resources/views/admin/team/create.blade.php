                        ...
                        <div class="card card-outline card-primary">
                            ...
                            <div class="card-body">
                                <div class="row g-3">
                                    ...
                                    @php $selectedServices = old('services', []); @endphp
                                    <div class="col-12">
                                            ...
                                            <div class="row g-2 align-items-end">
                                                ...
                                                <div class="col-md-2">
                                                    ...
                                                    <div class="mt-2 position-relative d-inline-block icon-preview-wrap d-none">
                                                        <img src="" alt="icon preview" style="width: 24px; height: 24px; object-fit: contain;">
                                                        <button type="button" class="btn btn-danger btn-sm clear-icon-btn position-absolute top-0 start-100 translate-middle rounded-circle p-0 d-flex align-items-center justify-content-center" style="width:16px;height:16px;font-size:10px;line-height:1;" title="Remove selected icon">&times;</button>
                                                    </div>
                                                </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (descEditorEl) {
        ...
        if (projectsTagContainer && projectsHiddenInputs) {
            // Shows a small "1, 2, 3..." badge on each selected tag, in submission
            // order, so the admin can see the order they're setting without guessing.
            function renumberProjects() {
                projectsTagContainer.querySelectorAll('.tag-order-badge').forEach(function (b) { b.remove(); });
                Array.from(projectsHiddenInputs.querySelectorAll('input')).forEach(function (input, index) {
                    const tag = projectsTagContainer.querySelector('.project-tag[data-id="' + input.value + '"]');
                    if (!tag) return;
    ...
    document.addEventListener('click', function (e) {
        if (!e.target.classList.contains('clear-icon-btn')) return;
        const row = e.target.closest('.expertise-row, .social-row');
